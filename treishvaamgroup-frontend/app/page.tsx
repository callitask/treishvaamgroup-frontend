...
export default function Home() {
  ...
  // Instead of fake business data, we simulate "Server Health" which is valid for any website.
  ...
  useEffect(() => {
    // Check connection speed (Real browser API)
    // @ts-ignore
    const conn = typeof navigator !== 'undefined' && navigator.connection ? navigator.connection : { downlink: 10 };
    const isHighSpeed = conn.downlink > 5;

    const interval = setInterval(() => {
      setMetrics(prev => {
        // Performance fluctuates slightly based on "network load" simulation
        const newPerf = Math.min(100, Math.max(92, prev.performance + (Math.random() * 2 - 1)));
        
        // Network Health is an average of the three
        const avg = Math.round((99.9 + 100 + newPerf) / 3);

        // Random latency jitter between 15ms and 45ms
        const latencyVal = Math.round(Math.random() * 30 + 15);

        return {
          uptime: 99.9, // Static standard
          security: 100, // Static standard
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="grid lg:grid-cols-2 gap-24 items-center">
            ...
            <div className="relative h-[700px] w-full flex items-center justify-center lg:justify-end perspective-[2000px]">
               ...
               {/* 1. Global Operations Card (Left) - Kept generic for "Future Reach" */}
